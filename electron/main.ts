...
// 🖥️ Linux-to-Linux (PC-to-PC) Connection State
...
const discoveredPcs = new Map<string, { hostname: string; ip: string; port: number; machineId: string; lastSeen: number; device?: 'linux' | 'android' }>();
  ...
  io.on('connection', (socket: any) => {
    ...
    socket.on('announce-device', (data: { machineId: string; hostname: string; device: 'android' }) => {
      ...
      if (paired) {
        ...
        // Show on Linux UI as well (initiator side)
    ...
    socket.on('request-pairing', (data: { machineId: string; hostname: string; pairingCode: string; secretKey: string }) => {
      ...
      // Check if Linux was the initiator for this socket (Android is accepting back)
      // In that case activePairingSocket === socket and the code matches
      if (activePairingSocket === socket &&
        socket.data.pairingData &&
        socket.data.pairingData.pairingCode === pairingCode) {
        // Auto-complete: Linux generated the code, Android accepted it — no extra Linux click needed
        console.log(`[Pairing] Android accepted Linux-initiated pairing from ${hostname}. Auto-completing.`);
        const clientIp = socket.handshake.address.replace('::ffff:', '');
      ...
      // show popup on Linux for the user to accept/reject.
...
ipcMain.on('connect-to-pc', (_event: any, { ip, machineId }: { ip: string; machineId: string }) => {
  ...
  // Check if the target is an Android device (it's already connected as a server socket)
  const discoveredEntry = discoveredPcs.get(ip);
  if (discoveredEntry && discoveredEntry.device === 'android') {
    // Find the Android socket by machineId in the server
    if (io) {
      const sockets = Array.from(io.sockets.sockets.values()) as any[];
      const androidSocket = sockets.find(s => s.data && s.data.machineId === machineId && s.data.device === 'android');
      if (androidSocket) {
        // Already paired? Nothing to do — they auto-authenticate.
        const paired = config.pairedDevices.find(d => d.machineId === machineId);
        if (paired) {
  ...
  // Linux-to-Linux: connect as a socket client
...
ipcMain.on('accept-pairing', () => {
  if (activePairingSocket && activePairingSocket.data.pairingData) {
    ...
    const deviceType = activePairingSocket.data.device || 'linux';
...
function readBestClipboardText(): string {
  if (process.platform === 'linux') {
    // Under Linux, prioritize command-line clipboard tools to avoid XWayland/Wayland stale cache bugs in Electron
  ...
  // Fallback to Electron's built-in clipboard API
